...
## Integração com NixOS

O DACC Station utiliza NixOS no console final. A lógica do mapper é independente da distribuição porque usa as interfaces de input do kernel Linux, mas a instalação deve ser convertida para configuração declarativa do NixOS.

A futura integração deverá declarar, no mínimo:

- `uinput` como módulo de kernel carregado no boot;
- `python3` com o pacote `evdev`;
- regra `udev` equivalente a `71-dacc-gamepad.rules`;
- serviço do mapper;
- ordenação do launcher para iniciar após o serviço do gamepad.

A configuração NixOS **ainda não foi validada no Raspberry Pi/ToofOS** e, portanto, não é apresentada neste guia como concluída.
